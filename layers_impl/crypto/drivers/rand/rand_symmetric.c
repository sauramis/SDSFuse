...
int rand_init(char* key, int key_size, int operation_mode, block_align_config config) {
...
off_t rand_get_file_size(const char* path, off_t original_size, struct fuse_file_info* fi_in, struct fuse_operations nextlayer) {
    ...
    if (last_incomplete_block_size > 0) {
        ...
        DEBUG_MSG("rand_get_file_size: rand_decode path %s size %d.\n", path, last_incomplete_block_size);
