...
int det_auth_init(char* key, int key_size, unsigned char* arg_iv, int iv_size, int tag_size, int operation_mode, block_align_config config) {
    DEBUG_MSG("det_auth_inti: %d -- %d -- %d -- %d\n", key_size, iv_size, tag_size, operation_mode);

    DET_AUTH_BLOCKSIZE = config.block_size;
    DET_AUTH_IV = arg_iv;
